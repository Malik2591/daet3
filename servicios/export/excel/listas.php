<?php
	...
	if(isset($_GET['id_grp'])){
		...
		*/
		...
		$objPHPExcel->getActiveSheet()->setCellValue('A1',$desCarrera);
		$objPHPExcel->getActiveSheet()->setCellValue('J3',$perGrp.' Semestre');
		$objPHPExcel->getActiveSheet()->setCellValue('B2', $materia);
		$objPHPExcel->getActiveSheet()->setCellValue('B3', $profesor);
		...
		header("Content-Disposition: attachment;filename=lista_grupo_$claveGrp.xls");
	...
	elseif(isset($_GET['general'])){
		...
		$objPHPExcel = new PHPExcel();
		$objReader = PHPExcel_IOFactory::createReader('Excel5');
		$objPHPExcel = $objReader->load("plantilla_general.xls");
		
		while($grupo = mysql_fetch_assoc($grupos)){
			$idGrupo = $grupo['id_grp'];
			$query = "SELECT * FROM Alumno INNER JOIN Alumno_has_Grupo ON Alumno_has_Grupo.Alumno_rfc_alm=Alumno.rfc_alm INNER JOIN Grupo ON Grupo.id_grp=Alumno_has_Grupo.Grupo_id_grp INNER JOIN Carrera ON Carrera.id_carrera=Grupo.Carrera_id_carrera WHERE Grupo.id_grp='$idGrupo'";
			$alumnos = mysql_query($query,$link);
			$total = mysql_num_rows($alumnos);
			
			if($total!=0){
				$objPHPExcel->setActiveSheetIndex($indiceHoja);
				$objPHPExcel->getActiveSheet()->setCellValue('A1',$grupo['des_grp']);
				$objPHPExcel->getActiveSheet()->setCellValue('A2','Total de alumnos: '.$total);
				...
				while($alumno = mysql_fetch_assoc($alumnos)){
					...
					$objPHPExcel->getActiveSheet()->setCellValue('A'.$i1, $matricula);
					$objPHPExcel->getActiveSheet()->setCellValue('B'.$i1, $nombre);
					$objPHPExcel->getActiveSheet()->setCellValue('C'.$i1, utf8_encode($alumno['des_grp']));
					$i1=$i1+1;
				}
				
				// Rename sheet
				$objPHPExcel->getActiveSheet()->setTitle($grupo['clave_grp']);
				$objPHPExcel->getActiveSheet()->getColumnDimension('A')->setAutoSize(true);
				$objPHPExcel->getActiveSheet()->getColumnDimension('B')->setAutoSize(true);
				$objPHPExcel->getActiveSheet()->getColumnDimension('C')->setAutoSize(true);
		...
		// Redirect output to a client’s web browser (Excel2007)
		header("Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
		header("Content-Disposition: attachment;filename=lista_general.xls");
		header("Cache-Control: max-age=0");
